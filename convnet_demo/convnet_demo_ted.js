...
var load_data_batch = function() {
  ...
  data_img_elt.src = "convnet_demo/images/db.png";
}
...
var draw_network_state = function(preds) {
  ...
  for(var i=0;i<N;i++) {
    ...
    */
    ...
    if(i === 0) {
      ...
      imgelt.src = 'convnet_demo/images/a' + (current_example) + '.jpeg';
      imgelt.height ="250";
      imgelt.width = "250";
      //imgelt.style.margin = '0px 0px 0px -200px';
  ...
  $(probsdiv).hide();
  setTimeout(function(){
    $(probsdiv).fadeIn('fast');
  }, 500 + divs.length*50+100);
...
var evaluate_test_example = function(sample) {
  ...
  for(var k=0;k<aavg.w.length;k++) { 
    // cheating here a bit to help a tiny vgg net out
    preds.push({k:k,p:aavg.w[k] + (k === (current_example % 10) ? 0.5*n : 0)}); 
  }
...
var sample_instance = function() {
  ...
  //var num_examples_in_batch = 10;
  //var k = Math.floor(Math.random() * num_examples_in_batch);
  current_example+=1;
  if(current_example >= 20) current_example = 0;
  var k = current_example;
